...
module.exports = ({ strapi }) => ({
  ...
  fetchAuthenticatedUser(id) {
    return strapi.query('plugin::users-permissions.user').findOne({ where: { id }, populate: ['role'] })
  },
   ...
   */
  async remove(params) {
    return await strapi.query('plugin::users-permissions.user').delete({ where: params })
  },
  isHashed(password) {
  ...
  async sendConfirmationEmail(user) {
    ...
    await strapi
      ...
      .send({
        ...
        templateId: 'd-c37b8cf4ff374bfe93df6e6a3c7a8058',
        dynamicTemplateData: {
          url:
            urlJoin(getAbsoluteServerUrl(strapi.config), apiPrefix, '/auth/email-confirmation') +
            '?confirmation=' +
            confirmationToken,
        },
      })
